...
def markets(matrix: Dict[Tuple[int, int], float]) -> dict[str, float]:
    ...
    for (home, away), probability in matrix.items():
        result["home_win" if home > away else "away_win" if away > home else "draw"] += probability
        total = home + away
        for threshold in (0.5, 1.5, 2.5, 3.5):
            result[f"over_{threshold}"] = result.get(f"over_{threshold}", 0.0) + (probability if total > threshold else 0.0)
        result["btts_yes"] = result.get("btts_yes", 0.0) + (probability if home > 0 and away > 0 else 0.0)
